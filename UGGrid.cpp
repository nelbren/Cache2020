...
void CUGGrid::DrawCellsIntern(CDC *dc,CDC *db_dc)
{
	...
	for(y = 0; y < m_GI->m_numberRows;y++)
	{
		...
		if(y >= (m_GI->m_numberRows-1)){
			int64_t origNumRows = m_GI->m_numberRows;
			int64_t newRow = y+1;
...
Returns:
	<none>
*****************************************************/
void CUGGrid::RedrawCell(int col,int64_t row)
{
...
Returns:
	<none>
*****************************************************/
void CUGGrid::RedrawRow(int64_t row)
{
...
Returns:
	<none>
*****************************************************/
void CUGGrid::RedrawRange(int startCol,int64_t startRow,int endCol,int64_t endRow)
{
	m_drawHint.AddHint(startCol,startRow,endCol,endRow);
	Moved();
}
...
void CUGGrid::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	int64_t curRow = m_GI->m_currentRow;
	int  JoinCellStartCol, JoinCellEndCol, colOff, rowOff;
	int64_t JoinCellStartRow, JoinCellEndRow;
