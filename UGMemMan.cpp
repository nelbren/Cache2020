...
Return
	...
	UG_ERROR	fail
****************************************************/
int CUGMem::GotoRow(int64_t row)
{
	if(m_rowInfo == NULL)
		return UG_ERROR;
...
Return
	...
	UG_ERROR	- failure
****************************************************/
int CUGMem::InsertRow(int64_t row)
{
...
Return
	UG_SUCCESS  - success
****************************************************/
int CUGMem::DeleteRow(int64_t row)
{
	//adjust the linked list if the list has an item at the insertion point
	if(GotoRow(row)==UG_SUCCESS)
	{
...
Return
	...
	UG_ERROR	- failure
****************************************************/
int CUGMem::DeleteCell(int col,int64_t row)
{
	if(GotoRow(row)==UG_SUCCESS)
	{
		if(GotoCol(col)==UG_SUCCESS)
...
Return
	...
	UG_ERROR	- failure
****************************************************/
int CUGMem::GetCell(int col,int64_t row,CUGCell *cell)
{
	//TODO
	/* 
...
Return
	...
	UG_ERROR	- failure
****************************************************/
int CUGMem::SetCell(int col,int64_t row,CUGCell *cell)
{
	UGMemRI *newrow;
	UGMemCI *newcol;
...
Return
	...
	UG_ERROR	- failure
****************************************************/
int CUGMem::FindFirst(CString *string,int *col,int64_t *row,long flags){
...
Return
	...
	UG_ERROR	- failure
****************************************************/
int CUGMem::FindNext(CString *string,int *col,int64_t *row,int flags)
{
	BOOL success = FALSE;
	CString tempStr;
	...
	int64_t lastRow = m_ctrl->GetNumberRows()-1;
	int  lastCol = m_ctrl->GetNumberCols()-1;
	int64_t searchLimit = lastRow+2;		// search with wrap
	int64_t rowCount = 0;
	BOOL colsDone = FALSE;
...
int CUGMem::SortBy(int64_t startRow, int64_t endRow, int *cols,int numCols,int flags)
{
	if(m_rowInfo == NULL)
		return UG_ERROR;
...
Return
	...
	UG_ERROR	- failure	
****************************************************/
int CUGMem::GetPrevNonBlankCol(int *col,int64_t row)
{
