...
public:
	...
	void ResetSizes(int64_t startRow, int64_t endRow, int startCol, int endCol);
...
public:
	...
	int VerifyTopRow(int64_t* newRow);
	int VerifyCurrentRow(int64_t* newRow);
	...
	int		StartEdit(int col,int64_t row,int key);
	int		ContinueEdit(int adjustCol,int64_t adjustRow);

	//*************** row and column *****************
	//int		SetNumberRows(int64_t rows,BOOL redraw = TRUE);
	int		SetNumberRows(int64_t rows, BOOL redraw = TRUE);
	//long	GetNumberRows();
	int64_t	GetNumberRows();
	...
	int		SetRowHeight(int64_t row,int height);
	int		GetRowHeight(int64_t row,int *height);
	int64_t		GetRowHeight(int64_t row);
	...
	int		GetRightCol();
	//long	GetBottomRow();
	int64_t	GetBottomRow();

	int		InsertCol(int col);
	int		AppendCol();
	int		DeleteCol(int col);
	int		InsertRow(int64_t row);
	int		AppendRow();
	int		DeleteRow(int64_t row);
	...
	//************* find/replace/sort *********************
	int FindFirst(CString *string,int *col,int64_t *row,long flags);
	int FindNext(CString *string,int *col,int64_t *row,int flags);
	...
	int		GetCell(int col,int64_t row,CUGCell *cell);
	int		GetCellIndirect(int col,int64_t row,CUGCell *cell);
	int		SetCell(int col,int64_t row,CUGCell *cell);
	int		DeleteCell(int col,int64_t row);
	...
	//cell joining
	int		JoinCells(int startcol,int64_t startrow,int endcol,int64_t endrow);
	int		UnJoinCells(int col,int64_t row);
	int		EnableJoins(BOOL state);

	//cell functions
	int		QuickSetText(int col,int64_t row,LPCTSTR string);
	int		QuickSetNumber(int col,int64_t row,double number);
	int		QuickSetMask(int col,int64_t row,LPCTSTR string);
	int		QuickSetLabelText(int col,int64_t row,LPCTSTR string);
	int		QuickSetTextColor(int col,int64_t row,COLORREF color);
	int		QuickSetBackColor(int col,int64_t row,COLORREF color);
	int		QuickSetAlignment(int col,int64_t row,short align);
	int		QuickSetBorder(int col,int64_t row,short border);
	int		QuickSetBorderColor(int col,int64_t row,CPen *pen);
	int		QuickSetFont(int col,int64_t row,CFont * font);
	int		QuickSetFont(int col,int64_t row,int index);
	int		QuickSetBitmap(int col,int64_t row,CBitmap * bitmap);
	int		QuickSetBitmap(int col,int64_t row,int index);
	int		QuickSetCellType(int col,int64_t row,long type);
	int		QuickSetCellTypeEx(int col,int64_t row,long typeEx);
	int		QuickSetHTextColor(int col,int64_t row,COLORREF color);
	int		QuickSetHBackColor(int col,int64_t row,COLORREF color);

	int		QuickSetRange(int startCol,int64_t startRow,int endCol,int64_t endRow,
				CUGCell* cell);

	int			QuickGetText(int col,int64_t row,CString *string);
	LPCTSTR		QuickGetText(int col,int64_t row);

	int		DuplicateCell(int destCol,int64_t destRow, int srcCol, int64_t srcRow);
	...
	int AdjustTopRow(int64_t adjust);
	int MoveCurrentRow(int flag);
	int AdjustCurrentRow(int64_t adjust);
	int GotoRow(int64_t row);
	int	SetTopRow(int64_t row);
	...
	int GotoCell(int col,int64_t row);


	//************* finding cells *********************
	int GetCellFromPoint(int x,int y,int *col,int64_t *row);
	int GetCellFromPoint(int x,int y,int *ptcol,int64_t *ptrow,RECT *rect);
	int GetAbsoluteCellFromPoint(int x,int y,int *ptcol,int64_t *ptrow);

	int GetCellRect(int col,int64_t row,RECT *rect);
	int GetCellRect(int *col,int64_t *row,RECT *rect);
	int GetRangeRect(int startCol,int64_t startRow,int endCol,int64_t endRow,RECT *rect);
	
	int GetJoinStartCell(int *col,int64_t *row);
	int GetJoinStartCell(int *col,int64_t *row,CUGCell *cell);
	int GetJoinRange(int *col,int64_t *row,int *col2,int64_t *row2);
	...
	//************* redrawing *********************
	int RedrawAll();
	int RedrawCell(int col,int64_t row);
	int RedrawRow(int64_t row);
	int RedrawCol(int col);
	int RedrawRange(int startCol,int64_t startRow,int endCol,int64_t endRow);
	void TempDisableFocusRect();


	//************* multi-select *********************
	int ClearSelections();
	int Select(int col,int64_t row);
	int SelectRange(int startCol,int64_t startRow,int endCol,int64_t endRow);
	int EnumFirstSelected(int *col,int64_t *row);
	int EnumNextSelected(int *col,int64_t *row);
	int EnumFirstBlock(int *startCol,int64_t *startRow,int *endCol,int64_t *endRow);
	int EnumNextBlock(int *startCol,int64_t *startRow,int *endCol,int64_t *endRow);
	BOOL IsSelected(int col,int64_t row,int *blockNum = NULL);
	...
	//************* printing ********************
	#ifdef UG_ENABLE_PRINTING
		int PrintInit(CDC * pDC, CPrintDialog* pPD, int startCol,int64_t startRow,
	...
	//movement and sizing
	virtual int  OnCanMove(int oldcol,int64_t oldrow,int newcol,int64_t newrow);
	virtual int  OnCanViewMove(int oldcol,int64_t oldrow,int newcol,int64_t newrow);
	virtual void OnHitBottom(int64_t numrows,int64_t rowspast,int64_t rowsfound);
	virtual void OnHitTop(int64_t numrows,int64_t rowspast);
	
	virtual int  OnCanSizeCol(int col);
	virtual void OnColSizing(int col,int *width);
	virtual void OnColSized(int col,int *width);
	virtual int  OnCanSizeRow(int64_t row);
	virtual void OnRowSizing(int64_t row,int *height);
	virtual void OnRowSized(int64_t row,int *height);
	...
	virtual void OnRowChange(int64_t oldrow,int64_t newrow);
	virtual void OnCellChange(int oldcol,int newcol,int64_t oldrow,int64_t newrow);
	virtual void OnLeftColChange(int oldcol,int newcol);
	virtual void OnTopRowChange(int64_t oldrow,int64_t newrow);
	virtual void OnViewMoved( int nScrolDir, long oldPos, long newPos );
	virtual void OnSelectionChanged(int startCol,int64_t startRow,int endCol,int64_t endRow,int blockNum);

	//mouse and key strokes
	virtual void OnLClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed);
	virtual void OnRClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed);
	virtual void OnDClicked(int col,int64_t row,RECT *rect,POINT *point,BOOL processed);
	virtual void OnMouseMove(int col,int64_t row,POINT *point,UINT nFlags,BOOL processed=0);
	virtual void OnTH_LClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed=0);
	virtual void OnTH_RClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed=0);
	virtual void OnTH_DClicked(int col,int64_t row,RECT *rect,POINT *point,BOOL processed=0);
	virtual void OnSH_LClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed=0);
	virtual void OnSH_RClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed=0);
	virtual void OnSH_DClicked(int col,int64_t row,RECT *rect,POINT *point,BOOL processed=0);
	...
	//virtual int OnCellTypeNotify(long ID,int col,int64_t row,long msg,LONG_PTR param);
	virtual int OnCellTypeNotify(long ID, int col, int64_t row, long msg, LONG_PTR param);

	//editing
    /*
	virtual int OnEditStart(int col, int64_t row,CWnd **edit);
	virtual int OnEditVerify(int col,int64_t row,CWnd *edit,UINT *vcKey);
	virtual int OnEditFinish(int col, int64_t row,CWnd *edit,LPCTSTR string,BOOL cancelFlag);
	virtual int OnEditContinue(int oldcol,int64_t oldrow,int* newcol,int64_t* newrow);
	virtual int OnEditKeyDown(int col,int64_t row,CWnd *edit,UINT *vcKey);
	virtual int OnEditKeyUp(int col,int64_t row,CWnd *edit,UINT *vcKey);
	*/
	...
	//menu notifications
	//virtual void OnMenuCommand(int col,int64_t row,int section,int item);
	//virtual int  OnMenuStart(int col,int64_t row,int section);

	virtual void OnMenuCommand(int col, int64_t row, int section, int item);
	virtual int  OnMenuStart(int col, int64_t row, int section);

	// Context help 
	//virtual DWORD OnGetContextHelpID( int col, int64_t row, int section );
	virtual DWORD OnGetContextHelpID(int col, int64_t row, int section);

	//hints
	//virtual int OnHint(int col,int64_t row,int section,CString *string);
	//virtual int OnVScrollHint(int64_t row,CString *string);
	virtual int OnHint(int col, int64_t row, int section, CString* string);
	virtual int OnVScrollHint(int64_t row, CString* string);
	...
	//drag and drop
		...
		virtual DROPEFFECT OnDragOver(COleDataObject* pDataObject,int col,int64_t row);
		virtual DROPEFFECT OnDragDrop(COleDataObject* pDataObject,int col,int64_t row);
	#endif
	...
	int StartMenu(int col,int64_t row,POINT *point,int section);
	...
	int EditCtrlFinished(LPCTSTR string,BOOL cancelFlag,
		BOOL continueFlag,int continueCol,int64_t continueRow);
	...
	CUGCellType * GetCellType(int col,int64_t row);
