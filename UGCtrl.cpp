...
void CUGCtrl::CalcTopRow(){
	...
	int64_t oldMaxRow = m_GI->m_maxTopRow;

	//calc the max top row
	//int64_t row;
	int64_t row;
	int64_t height = m_GI->m_gridHeight - m_GI->m_lockRowHeight;
...
void CUGCtrl::AdjustTopRow(){
	...
	if(m_GI->m_uniformRowHeightFlag == FALSE){
		...
		//int64_t row = m_GI->m_dragRow - height / m_GI->m_defRowHeight + 1;
		int64_t row = m_GI->m_dragRow - height / m_GI->m_defRowHeight + 1;
...
void CUGCtrl::Moved()
{
	...
	int64_t nOldBottomRow = GetBottomRow();
	...
	int64_t nNewBottomRow = GetBottomRow();
	...
	else if ( m_GI->m_lastLeftCol != m_GI->m_leftCol || nOldRightCol != nNewRightCol )
	{
		...
		{	// user scrolled right, send notifications to 
			for(int64_t nIndex = m_GI->m_topRow; nIndex < nNewBottomRow; nIndex ++ )
			{
				// old left column ...
				m_cell.ClearAll();
...
GetCellType
	...
	Return
		...
		is returned
****************************************************/
//CUGCellType * CUGCtrl::GetCellType(int col,int64_t row)
CUGCellType* CUGCtrl::GetCellType(int col, int64_t row)
{
	m_cell.ClearAll();
...
****************************************************/
int CUGCtrl::VerifyTopRow(int64_t* newRow){
...
/***************************************************
	...
	Return
****************************************************/
int CUGCtrl::VerifyCurrentRow(int64_t* newRow){
...
int CUGCtrl::VerifyCurrentCol(int* newCol){
	...
	if(m_GI->m_enableJoins){
		...
		if(m_cell.IsPropertySet(UGCELL_JOIN_SET)){
			int startCol = *newCol;
			int64_t startRow = m_GI->m_dragRow;
			int endCol;
			int64_t endRow;
			GetJoinRange(&startCol,&startRow,&endCol,&endRow);
...
AdjustTopRow
  ...
  returns 
	...
	UG_SUCCESS	success
****************************************************/
int CUGCtrl::AdjustTopRow(int64_t adjust){
...
AdjustCurrentRow
  ...
  returns 
	...
	UG_SUCCESS	success
****************************************************/
int CUGCtrl::AdjustCurrentRow(int64_t adjust){
...
GotoRow
  ...
  returns 
	...
	UG_SUCCESS	success
****************************************************/
//int CUGCtrl::GotoRow(int64_t row){
int CUGCtrl::GotoRow(int64_t row) {
...
SetTopRow
  ...
  returns 
	...
	UG_SUCCESS	success
****************************************************/
//int CUGCtrl::SetTopRow(int64_t row){
int CUGCtrl::SetTopRow(int64_t row) {
...
GotoCell
  ...
  returns 
	...
	UG_SUCCESS	success
****************************************************/
//int CUGCtrl::GotoCell(int col,int64_t row)
int CUGCtrl::GotoCell(int col, int64_t row)
{	
...
GetCellFromPoint
  ...
  Return
	...
	UG_SUCCESS	success
****************************************************/
int CUGCtrl::GetCellFromPoint(int x,int y,int *ptcol,int64_t *ptrow){
...
GetCellFromPoint
  ...
  Return
	...
	UG_SUCCESS	success
****************************************************/
int CUGCtrl::GetCellFromPoint(int px,int py,int *ptcol,int64_t *ptrow,RECT *rect){
...
****************************************************/
int CUGCtrl::GetAbsoluteCellFromPoint(int x,int y,int *ptcol,int64_t *ptrow){
...
GetCellRect
  ...
  Return
	...
	UG_SUCCESS	success
****************************************************/
//int CUGCtrl::GetCellRect(int col,int64_t row,RECT *rect){
int CUGCtrl::GetCellRect(int col, int64_t row, RECT * rect) {
...
****************************************************/
int CUGCtrl::GetRangeRect(int startCol,int64_t startRow,int endCol,int64_t endRow,RECT *rect){
...
cell of that block
****************************************************/
int CUGCtrl::GetJoinStartCell(int *col,int64_t *row){
...
cell of that block
****************************************************/
int CUGCtrl::GetJoinStartCell(int *col,int64_t *row,CUGCell *cell){
...
****************************************************/
int CUGCtrl::GetJoinRange(int *col,int64_t *row,int *col2,int64_t *row2){
...
StartEdit
  ...
  return
	...
	UG_SUCCESS	success
****************************************************/
int	CUGCtrl::StartEdit(int col,int64_t row,int key)
{
...
ContinueEdit
  ...
  return
	...
	UG_SUCCESS	success
****************************************************/
int	CUGCtrl::ContinueEdit(int adjustCol,int64_t adjustRow){
...
****************************************************/
int CUGCtrl::GetCell(int col,int64_t row,CUGCell * cell)
{
...
GetCellIndirect
  ...
  return
		...
		UG_ERROR	cell not found
****************************************************/
int	CUGCtrl::GetCellIndirect(int col,int64_t row,CUGCell *cell){
...
SetCell
  ...
  return
	...
	UG_ERROR	cell not found
****************************************************/
//int	CUGCtrl::SetCell(int col,int64_t row,CUGCell *cell){
int	CUGCtrl::SetCell(int col, int64_t row, CUGCell * cell) {
...
****************************************************/
int	CUGCtrl::DeleteCell(int col,int64_t row){
...
/***************************************************
****************************************************/
int	CUGCtrl::JoinCells(int startCol,int64_t startRow,int endCol,int64_t endRow){
...
/***************************************************
****************************************************/
int	CUGCtrl::UnJoinCells(int col,int64_t row){
...
/***************************************************
****************************************************/
int	CUGCtrl::DuplicateCell(int destCol,int64_t destRow, int srcCol, int64_t srcRow){
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickGetText(int col,int64_t row,CString *string){
...
/***************************************************
****************************************************/
LPCTSTR	CUGCtrl::QuickGetText(int col,int64_t row)
{
	m_cell.ClearAll();
	GetCellIndirect(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetText(int col,int64_t row,LPCTSTR string)
{
	ASSERT(string);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetNumber(int col,int64_t row,double number)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetMask(int col,int64_t row,LPCTSTR string)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetLabelText(int col,int64_t row,LPCTSTR string)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetTextColor(int col,int64_t row,COLORREF color)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetHTextColor(int col,int64_t row,COLORREF color)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetBackColor(int col,int64_t row,COLORREF color)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetHBackColor(int col,int64_t row,COLORREF color)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetBitmap(int col,int64_t row,CBitmap * bitmap)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetBitmap(int col,int64_t row,int index){
	return QuickSetBitmap(col,row,GetBitmap(index));
}
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetFont(int col,int64_t row,CFont * font)
{
	ASSERT(font);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetFont(int col,int64_t row,int index)
{
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetAlignment(int col,int64_t row,short align)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetBorder(int col,int64_t row,short border)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetBorderColor(int col,int64_t row,CPen *pen)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetCellType(int col,int64_t row,long type)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
/***************************************************
****************************************************/
int	CUGCtrl::QuickSetCellTypeEx(int col,int64_t row,long typeEx)
{
	m_cell.ClearAll();
	GetCell(col,row,&m_cell);
...
****************************************************/
int	CUGCtrl::QuickSetRange(int startCol,int64_t startRow,int endCol,int64_t endRow,
						   CUGCell *cell){
...
SetNumberRow
  ...
  return
		...
		UG_ERROR	fail
****************************************************/
int	CUGCtrl::SetNumberRows(int64_t rows,BOOL redraw){
	...
	int64_t oldNumRows = m_GI->m_numberRows;
	...
	if(m_GI->m_uniformRowHeightFlag == FALSE){
		//create the array
		int64_t *tempRowHeights = new int64_t[rows];
		
		//copy the old row heights over
		for(int64_t loop = 0;loop < rows ; loop++){
			if(loop < oldNumRows)
...
int CUGCtrl::SetColWidth(int col,int width)
{
	...
	if ( IsWindowVisible() == TRUE && m_GI->m_paintMode == TRUE )
	{
		...
		for( int64_t nIndex = GetTopRow(); nIndex < GetBottomRow(); nIndex++ )
		{
	...
	//uniform row height off
		...
		if(m_GI->m_rowHeights == NULL){
			if(m_GI->m_uniformRowHeightFlag && m_GI->m_numberRows >0){
				m_GI->m_rowHeights = new int64_t[m_GI->m_numberRows];
				for(int64_t row = 0;row <m_GI->m_numberRows;row++){
...
***************************************************/
int CUGCtrl::SetRowHeight( int64_t row,int height )
{
...
GetRowHeight
  ...
  return
	...
	UG_ERROR	fail - out of range
***************************************************/
int CUGCtrl::GetRowHeight(int64_t row,int *height)
{
	if( row >= m_GI->m_numberRows )
		return UG_ERROR;
...
/***************************************************
	...
	Return
***************************************************/
int64_t CUGCtrl::GetRowHeight(int64_t row){
...
/***************************************************
	...
	Return
***************************************************/
int	CUGCtrl::InsertRow(int64_t row){
...
/***************************************************
	...
	Return
***************************************************/
int	CUGCtrl::DeleteRow(int64_t row){
...
/***************************************************
	...
	Return
***************************************************/
int CUGCtrl::FindFirst(CString *string,int *col,int64_t *row,long flags){
...
/***************************************************
	...
	Return
***************************************************/
int CUGCtrl::FindNext(CString *string,int *col, int64_t *row,int flags){
	*col = m_GI->m_colInfo[*col].colTranslation;
...
***************************************************/
int CUGCtrl::RedrawCell(int col,int64_t row){
	if(col < 0){
...
***************************************************/
int CUGCtrl::RedrawRow(int64_t row){
...
RedrawRange
	...
	grid
***************************************************/
int CUGCtrl::RedrawRange(int startCol,int64_t startRow,int endCol,int64_t endRow){
...
Select
  ...
  return
	...
	UG_ERROR	fail - out of range
****************************************************/
int CUGCtrl::Select(int col,int64_t row){
...
/***************************************************
****************************************************/
BOOL CUGCtrl::IsSelected(int col,int64_t row,int *blockNum){
...
SelectRange
  ...
  return
	...
	UG_ERROR	fail - out of range
****************************************************/
int CUGCtrl::SelectRange(int startCol,int64_t startRow,int endCol,int64_t endRow)
{
...
EnumFirstSelected
  ...
  return
	...
	UG_ERROR	fail
****************************************************/
int CUGCtrl::EnumFirstSelected(int *col,int64_t *row){
...
****************************************************/
int CUGCtrl::EnumNextSelected(int *col,int64_t *row){
...
EnumFirstBlock
  ...
  return
	...
	UG_ERROR	fail
****************************************************/
int CUGCtrl::EnumFirstBlock(int *startCol,int64_t *startRow,int *endCol,int64_t *endRow){
...
EnumNextBlock
  ...
  return
	...
	UG_ERROR	no more items selected
****************************************************/
int CUGCtrl::EnumNextBlock(int *startCol,int64_t *startRow,int *endCol,int64_t *endRow){
...
//		DROPEFFECT_COPY - allow drag and drop for copying
****************************************************/
DROPEFFECT  CUGCtrl::OnDragOver(COleDataObject* pDataObject,int col,int64_t row){
	UNREFERENCED_PARAMETER(*pDataObject);
...
//		DROPEFFECT_COPY - allow drag and drop for copying
****************************************************/
DROPEFFECT  CUGCtrl::OnDragDrop(COleDataObject* pDataObject,int col,int64_t row){
	UNREFERENCED_PARAMETER(*pDataObject);
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	return DROPEFFECT_NONE;
...
****************************************************/
int CUGCtrl::OnCanMove(int oldcol,int64_t oldrow,int newcol,int64_t newrow){
...
****************************************************/
int CUGCtrl::OnCanViewMove(int oldcol,int64_t oldrow,int newcol,int64_t newrow){
	UNREFERENCED_PARAMETER(oldcol);
	UNREFERENCED_PARAMETER(oldrow);
	UNREFERENCED_PARAMETER(newcol);
	UNREFERENCED_PARAMETER(newrow);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnHitBottom(int64_t numrows,int64_t rowspast,int64_t rowsfound){
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnHitTop(int64_t numrows,int64_t rowspast){
	UNREFERENCED_PARAMETER(numrows);
	UNREFERENCED_PARAMETER(rowspast);
}
...
Return:
	...
	FALSE - to prevent sizing
****************************************************/
int  CUGCtrl::OnCanSizeRow(int64_t row){
	UNREFERENCED_PARAMETER(row);
	return TRUE;
}
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnRowSizing(int64_t row,int *height){
	UNREFERENCED_PARAMETER(row);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnRowSized(int64_t row,int *height){
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*height);
}
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnRowChange(int64_t oldrow,int64_t newrow){
	UNREFERENCED_PARAMETER(oldrow);
	UNREFERENCED_PARAMETER(newrow);
}
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnCellChange(int oldcol,int newcol,int64_t oldrow,int64_t newrow){
	UNREFERENCED_PARAMETER(oldcol);
	UNREFERENCED_PARAMETER(newcol);
	UNREFERENCED_PARAMETER(oldrow);
	UNREFERENCED_PARAMETER(newrow);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnTopRowChange( int64_t oldrow, int64_t newrow )
{
	UNREFERENCED_PARAMETER(oldrow);
	UNREFERENCED_PARAMETER(newrow);
}
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnSelectionChanged(int startCol,int64_t startRow,int endCol,int64_t endRow,int blockNum)
{
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnLClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnRClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(updn);
	UNREFERENCED_PARAMETER(*rect);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnDClicked(int col,int64_t row,RECT *rect,POINT *point,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*rect);
	UNREFERENCED_PARAMETER(*point);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnMouseMove(int col,int64_t row,POINT *point,UINT nFlags,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*point);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnTH_LClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(updn);
	UNREFERENCED_PARAMETER(*rect);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnTH_RClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(updn);
	UNREFERENCED_PARAMETER(*rect);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnTH_DClicked(int col,int64_t row,RECT *rect,POINT *point,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*rect);
	UNREFERENCED_PARAMETER(*point);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnSH_LClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(updn);
	UNREFERENCED_PARAMETER(*rect);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnSH_RClicked(int col,int64_t row,int updn,RECT *rect,POINT *point,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(updn);
	UNREFERENCED_PARAMETER(*rect);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnSH_DClicked(int col,int64_t row,RECT *rect,POINT *point,BOOL processed){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*rect);
	UNREFERENCED_PARAMETER(*point);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnGetCell(int col,int64_t row,CUGCell *cell){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
...
Return:
	<none>
****************************************************/
//void CUGCtrl::OnSetCell(int col,int64_t row,CUGCell *cell){	
void CUGCtrl::OnSetCell(int col, int64_t row, CUGCell * cell) {
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*cell);
...
****************************************************/
int CUGCtrl::OnCellTypeNotify(long ID,int col,int64_t row,long msg,LONG_PTR param){
	UNREFERENCED_PARAMETER(ID);
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(msg);
...
****************************************************/
int CUGCtrl::OnEditStart(int col, int64_t row,CWnd **edit){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
...
****************************************************/
int CUGCtrl::OnEditVerify(int col,int64_t row,CWnd *edit,UINT *vcKey){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*edit);
	UNREFERENCED_PARAMETER(*vcKey);
...
****************************************************/
int CUGCtrl::OnEditKeyDown(int col,int64_t row,CWnd *edit,UINT *vcKey){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*edit);
	UNREFERENCED_PARAMETER(*vcKey);
...
int CUGCtrl::OnEditKeyUp(int col,int64_t row,CWnd *edit,UINT *vcKey){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*edit);
	UNREFERENCED_PARAMETER(*vcKey);
...
int CUGCtrl::OnEditFinish(int col, int64_t row,CWnd* edit,LPCTSTR string,BOOL cancelFlag){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*edit);
...
****************************************************/
int CUGCtrl::OnEditContinue(int oldcol,int64_t oldrow,int* newcol,int64_t* newrow){
	UNREFERENCED_PARAMETER(oldcol);
	UNREFERENCED_PARAMETER(oldrow);
	UNREFERENCED_PARAMETER(*newcol);
	UNREFERENCED_PARAMETER(*newrow);
...
Return:
	<none>
****************************************************/
void CUGCtrl::OnMenuCommand(int col,int64_t row,int section,int item){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(section);
...
****************************************************/
int CUGCtrl::OnMenuStart(int col,int64_t row,int section){
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(section);
	return TRUE;
...
****************************************************/
int CUGCtrl::LockRows(int64_t numRows){
...
Return
	...
	3			- menu failed
****************************************************/
int CUGCtrl::StartMenu(int col,int64_t row,POINT *point,int section)
{
...
****************************************************/
int CUGCtrl::PrintInit(CDC * pDC, CPrintDialog* pPD, int startCol,int64_t startRow,
			  int endCol,int64_t endRow){
...
****************************************************/
int CUGCtrl::OnHint(int col,int64_t row,int section,CString *string){
	UNREFERENCED_PARAMETER(section);
...
Return:
	...
	FALSE - to prevent the hint from showing
****************************************************/
int CUGCtrl::OnVScrollHint(int64_t row,CString *string){
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*string);
	return TRUE;
}
...
****************************************************/
DWORD CUGCtrl::OnGetContextHelpID( int col, int64_t row, int section )
{
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(section);
...
void CUGCtrl::ResetSizes(int64_t startRow, int64_t endRow, int startCol, int endCol)
{
	const int64_t rows = GetNumberRows();
	const int cols = GetNumberCols();
