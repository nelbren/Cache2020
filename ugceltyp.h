...
public:
	...
	virtual void DrawText(CDC *dc,RECT *rect,int offset,int col,int64_t row,CUGCell *cell, int selected,int current);
	...
	virtual int GetCellOverlapInfo(CDC* dc,int col,int64_t row,int *overlapCol,CUGCell *cell);
...
public:
	...
	virtual BOOL OnLClicked(int col,int64_t row,int updn,RECT *rect,POINT *point);
	virtual BOOL OnRClicked(int col,int64_t row,int updn,RECT *rect,POINT *point);
	virtual BOOL OnDClicked(int col,int64_t row,RECT *rect,POINT *point);

	virtual BOOL OnKeyDown(int col,int64_t row,UINT *vcKey);
	virtual BOOL OnKeyUp(int col,int64_t row,UINT *vcKey);
	virtual BOOL OnCharDown(int col,int64_t row,UINT *vcKey);

	virtual BOOL OnMouseMove(int col,int64_t row,POINT *point,UINT nFlags);
	
	virtual void OnDraw(CDC *dc,RECT *rect,int col,int64_t row,CUGCell *cell,int selected,int current);
	virtual BOOL OnDrawFocusRect(CDC *dc,RECT *rect,int col,int row);

#ifdef UG_ENABLE_PRINTING
	virtual void OnDrawPrint(CDC *dc,RECT *rect,int col,int64_t row,CUGCell *cell,int selected,int current);
	virtual void OnPrint(CDC *dc,RECT *rect,int col,int64_t row,CUGCell *cell);
#endif

	virtual void OnSetFocus(int col,int64_t row,CUGCell *cell);
	virtual void OnKillFocus(int col,int64_t row,CUGCell *cell);

	virtual void OnChangedCellWidth(int col, int64_t row, int* width);
	virtual void OnChangingCellWidth(int col, int64_t row, int* width);
	virtual void OnChangedCellHeight(int col, int64_t row, int* height);
	virtual void OnChangingCellHeight(int col, int64_t row, int* height);

	virtual void GetBestSize(CDC *dc,CSize *size,CUGCell *cell);

	virtual void OnScrolled(int col,int64_t row,CUGCell *cell);
