...
{
	//int rows = 40;
	//int64_t rows = 17179869184;
	int64_t rows = 16777216;
	//m_data2[0] = -1;
	//tmparray[0] = 64;
	//m_data.SetSize( rows );
	// populate the array with random data
	for (int64_t yIndex = 0; yIndex < rows; yIndex ++ )
	{
		for ( int xIndex = 0; xIndex < COLS; xIndex ++ )
		{
			//m_data[yIndex].m_colData[xIndex] = rand()%100;
			m_data[yIndex][xIndex] = rand() % 100;
		}
	}
...
//	Return:
//		<none>
void MyCug::OnDClicked(int col,int64_t row,RECT *rect,POINT *point,BOOL processed)
{
...
//	Return:
//		<none>
//void MyCug::OnGetCell(int col,int64_t row,CUGCell *cell)
void MyCug::OnGetCell(int col, int64_t row, CUGCell* cell)
{
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
...
//	Return:
//		<none>
void MyCug::OnSetCell(int col, int64_t row, CUGCell *cell)
{
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*cell);
	...
	if ( col >= 0 && row >= 0 )
	{
		...
		m_data[row][col] = (int)cell->GetNumber();
	}
}
...
int MyCug::OnEditVerify(int col, int64_t row,CWnd *edit,UINT *vcKey)
{
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
