...
protected:
	...
	int64_t			m_bottomRow;	//the row drawn at the bottom

	int				m_isSizing;			//sizing flag
	int				m_canSize;			//sizing flag
	int				m_colOrRowSizing;	// 0 - col 1- row
	int64_t			m_sizingColRow;		// column or row number being sized
	...
	int GetCellRect(int col,int64_t row,RECT *rect);
	int GetCellFromPoint(CPoint *point,int *col,int64_t *row,RECT *rect);

	void DrawCellsIntern(CDC *dc);

	// v7.2 - update 04 - added for WM_PRINT - TD
